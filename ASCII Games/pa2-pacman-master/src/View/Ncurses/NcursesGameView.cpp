...
NcursesGameView::NcursesGameView(const Game *game)
    ...
    sizeY = 2 + game->getMap().getSizeY() * ViewConfig::SQUARE_SIZE_Y + ViewConfig::STATUS_BAR_SIZE_X + 1;
...
void NcursesGameView::drawInstructions() const {
    std::string text = "CONTROLS: ARROWS to Move, Q to Quit";
    wattron(window, COLOR_PAIR(NcursesUtils::colorCode(ViewConfig::STATUS_BAR_TEXT_COLOR)) | A_DIM);
    mvwprintw(window, static_cast<int>(sizeY - 2), static_cast<int>((sizeX - text.length()) / 2), text.c_str());
    wattroff(window, COLOR_PAIR(NcursesUtils::colorCode(ViewConfig::STATUS_BAR_TEXT_COLOR)) | A_DIM);
}
