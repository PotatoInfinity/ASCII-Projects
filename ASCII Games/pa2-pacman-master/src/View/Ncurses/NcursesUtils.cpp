...
namespace NcursesUtils
{
    ...
    const InputKey getPressedKey(WINDOW *window) {
        ...
        if (c == 10) {
            ...
            // Check if ESC is actually an arrow key encoding escape sequence, or just ESC pressed alone
            if (c == 27) {
                // If there's no immediate sequence following ESC, treat it as ESC (QUIT)
                // We use halfdelay or nodelay with wgetch to check if 91 follows.
                // Since window has wtimeout(window, 0) set (nodelay mode), we can check immediately.
                int32_t next_char = wgetch(window);
                if (next_char == 91) {
                    int32_t arrow_char = wgetch(window);
                    switch (arrow_char) {
                        case 'A':
                            return InputKey::UP;
                        case 'B':
